...
export class MapViewComponent implements AfterViewInit, OnDestroy, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    // Whenever locations change, update markers
    if (changes['locations'] && this.map) {
      console.log('Locations changed:', this.locations);
  ...
  private initMap(): void {
    // Create the map initially with a default center
    this.map = L.map('map', {
      center: [22.5726, 88.3639], // temporary center (Kolkata)
      zoom: 13,
      preferCanvas: false
    });
    ...
    // 🟢 After creating map, center it based on actual locations
  ...
  private addMarkers(): void {
      ...
      const marker = L.marker([loc.lat, loc.lng])
        ...
        .bindPopup(
          `<div>
            ...
            <p>${loc.description ?? ''}</p>
            <a href="/event/${loc.id}">View details</a>
          </div>`
        );
    ...
    // 🟢 Adjust map view dynamically after adding markers
  ...
  private centerMapToLocations(): void {
